...
public class ContentUpDownArgsGeneratorAll implements PvUvArgsGenerator {
	...
	public List<PvUvArgs> execute(String line) {
		...
		if (m >= 0) {
			PvUvArgs cell = new PvUvArgs(hm.get("docId"), hm.get("userId"),
					EditorEvaluationKeyBuilder.getColumnName(StatisticsIndicator.contentUp, Platform.all, ShareBackChannel_EE.all), 
							m);
			al.add(cell);
		} else {
			PvUvArgs cell = new PvUvArgs(hm.get("docId"),
					hm.get("userId"),
					EditorEvaluationKeyBuilder.getColumnName(StatisticsIndicator.contentDown, Platform.all, ShareBackChannel_EE.all),
					Math.abs(m));
			al.add(cell);
		}
		return al;
