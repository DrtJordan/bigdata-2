...
public class DailyUrlInfoVector3MR extends MRJob {
	...
	public static class DailyUrlInfoReducer extends Reducer<StringLongSemiHash, HashMapStringStringWritable, Text, HashMapStringStringWritable> {
		...
		private String prePureUrl = null;
		private String curPureUrl = null;
		...
		protected void cleanup(Context context) throws IOException, InterruptedException {
			if(commonInfoMap.size()!=0){
				...
				HashMapStringStringWritable pureUrlOutputCommonInfoMap = new HashMapStringStringWritable();
					...
					pureUrlOutputCommonInfoMap.setHm( preparePureUrlResult(commonInfoMap,prePureUrl));
					try {
		...
		protected void reduce(StringLongSemiHash key, Iterable<HashMapStringStringWritable> values, Context context) throws IOException, InterruptedException {
			if (pre==null && cur == null &&prePureUrl == null && prePureUrl ==null) {
				pre = key.getSecond();
				prePureUrl = key.getFirst();
			}
			
			cur = key.getSecond();
			curPureUrl = key.getFirst();
			
			
			
			if (!curPureUrl.equals(prePureUrl)){	
				...
				if (cur == 0){
					...
					if (valueCommonInfoMap.getHm().get("url")!=null){
						valueCommonInfoMap.setHm(prepareUrlResult(valueCommonInfoMap.getHm(),prePureUrl));
						try {
						...
						valueCommonInfoMap.setHm(preparePureUrlResult(valueCommonInfoMap.getHm(),prePureUrl));
						try {
